...
// FASE 3: Importar Storage Multi-Capa desde storage_new.js
// ====================================================================
// Storage proporciona 3 capas:
// - CAPA 1: sessionBuffer (RAM - inmediato)
// - CAPA 2: Checkpoints (chrome.storage - recuperación)
// - CAPA 3: Sync a memoria_eva.jsonl (append-only + backup)
import { Storage } from './storage_new.js';
import { NeurobitTracker } from './neurobit_tracker.js';
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // FASE 3: Usar Storage.recoverCheckpoint() para CAPA 2 + CAPA 3
  (async () => {
    try {
      // Primero intenta recuperar desde CAPA 2 (Storage multi-capa)
      const checkpoint = await Storage.recoverCheckpoint();
      if (checkpoint && checkpoint.content && checkpoint.content.length > 0) {
        messageInput.value = checkpoint.content;
        estado.innerText = "🔄 Mensaje recuperado (Storage multi-capa)";
        console.log("[POPUP-RECOVERY] ✅ Checkpoint recuperado desde Storage:", checkpoint.label);
        setTimeout(() => {
      ...
      console.warn("[POPUP-RECOVERY] ⚠️ Error recuperando desde Storage:", err);
    }

    // Fallback: Si Storage.recoverCheckpoint() falla, intenta legacy method
    try {
      ...
      if (lastMessage && messageInput.value === "") {
        ...
        estado.innerText = "🔄 Mensaje recuperado (backup legacy)";
        console.log("[POPUP-RECOVERY] ✅ Fallback recuperado desde NeurobitTracker");
        setTimeout(() => {
          estado.innerText = "✅ Listo";
        }, 2000);
      }
    } catch (err) {
      console.warn("[POPUP-RECOVERY] ⚠️ Error en fallback legacy:", err);
    }
  })();

  // ========== BUTTON: RECUPERAR (FASE 3) ==========
  // Botón manual para recuperar sesión anterior desde Storage multi-capa
  const btnRecuperar = document.getElementById("recuperarBtn");
  if (btnRecuperar) {
    btnRecuperar.addEventListener("click", async () => {
      estado.innerText = "⏳ Recuperando sesión anterior...";
      console.log("[POPUP-RECOVER-BTN] 🔄 Iniciando recuperación manual...");
      
      try {
        // Intentar recuperar desde CAPA 2 (Storage multi-capa)
        const checkpoint = await Storage.recoverCheckpoint();
        if (checkpoint && checkpoint.content && checkpoint.content.length > 0) {
          messageInput.value = checkpoint.content;
          estado.innerText = `✅ Sesión recuperada (${checkpoint.content.length} caracteres)`;
          console.log("[POPUP-RECOVER-BTN] ✅ Checkpoint restaurado:", checkpoint.label);
          
          NeurobitTracker.logEvent("MANUAL_RECOVERY_SUCCESS", {
            checkpoint_label: checkpoint.label,
            content_length: checkpoint.content.length,
          ...
          estado.innerText = "⚠️ No hay sesión previa para recuperar";
          console.warn("[POPUP-RECOVER-BTN] No hay checkpoint disponible");
          
          NeurobitTracker.logEvent("MANUAL_RECOVERY_NO_DATA", {
            timestamp: new Date().toISOString()
          });
        }
      } catch (err) {
        console.error("[POPUP-RECOVER-BTN] ❌ Error recuperando:", err);
        estado.innerText = "❌ Error al recuperar sesión";
        
        NeurobitTracker.logEvent("MANUAL_RECOVERY_ERROR", {
  ...
  btnGuardar.addEventListener("click", () => {
    ...
    // FASE 3: Guardar en Storage multi-capa (CAPA 2)
    // =========================================================
    Storage.saveCheckpoint(`popup_guardar_${Date.now()}`, {
      destinatario: destinatario,
      text: mensaje,
      source: 'popup-manual',
      timestamp: new Date().toISOString()
    }).catch(err => {
      console.warn("[GUARDAR] ⚠️ Error al guardar en Storage (no bloqueante):", err);
    });
    ...
    chrome.storage.local.get({ bitacora: [] }, (res) => {
      try {
        ...
        chrome.storage.local.set({ bitacora }, () => {
          ...
          estado.innerText = "✅ Guardado en memoria + Storage multi-capa";
  ...
  // ========================================
  // UTILIDAD: Convertir cualquier valor a string seguro
  // ========================================
  function ensureString(value) {
    if (value === null || value === undefined) return "";
    if (typeof value === 'string') return value;
    if (typeof value === 'object' && value.content && typeof value.content === 'string') return value.content;
    if (typeof value === 'object' && value.text && typeof value.text === 'string') return value.text;
    if (typeof value === 'object' && value.body && typeof value.body === 'string') return value.body;
    if (typeof value === 'object') return JSON.stringify(value);
    return String(value);
  }

  // ========================================
  // PASO 4: LISTENER PARA EVA_LAST_TEXT (desde content.js)
  // ========================================
  // Recibe el último texto capturado desde clipboard/content.js
  window.addEventListener("message", (event) => {
    if (event.source !== window) return; // Solo aceptar mensajes de la misma ventana
    
    if (event.data && event.data.type === "EVA_LAST_TEXT" && event.data.text) {
      const lastText = ensureString(event.data.text);
      if (lastText && lastText.length > 0 && messageInput.value === "") {
        messageInput.value = lastText;
        estado.innerText = "✅ Último mensaje capturado desde clipboard";
        console.log("[POPUP] ✅ EVA_LAST_TEXT recibido y aplicado:", lastText.substring(0, 50) + "...");
        
        NeurobitTracker.logEvent("EVA_LAST_TEXT_RECEIVED", {
