...
window.addEventListener("message", (event) => {
  if (event.data.type === "EVA_SEND") {
    ...
    if (box) {
      // CRÍTICO: usar textContent en lugar de innerHTML para que React lo detecte
      box.textContent = msg;
      
      // Disparar eventos de input/change para que React se entere del cambio
      box.dispatchEvent(new Event("input", { bubbles: true }));
      box.dispatchEvent(new Event("change", { bubbles: true }));

      // Esperar a que React reconcile el estado (120ms)
      setTimeout(() => {
        const sendBtn = document.querySelector("button[data-testid='send-button']");
        if (sendBtn) {
          sendBtn.click();
          console.log("[EVA] ✅ Mensaje inyectado y botón presionado");
        } else {
          console.warn("[EVA] ⚠️ Botón de envío no encontrado");
        }
      }, 120);
    } else {
      console.warn("[EVA] ⚠️ Textarea contenteditable no encontrado");
    }
  }
});
...
async function getLastResponseText() {
  ...
  // Esperar un poco para que se complete la copia
  await new Promise(resolve => setTimeout(resolve, 100));

  // Ahora intentamos leer desde el portapapeles (ahora es async)
  try {
    const text = await navigator.clipboard.readText();
    return typeof text === 'string' ? text : JSON.stringify(text);
  } catch (err) {
    console.error('[EVA] Error reading clipboard:', err);
    return null;
  }
}
