...
const NeurobitTracker = (() => {
  ...
  function logEvent(eventType, data) {
    ...
    // FASE 2: Usar Storage multi-capa (CAPA 1 + CAPA 2)
    // =========================================================
    // CAPA 1: Agregar al buffer de sesión (RAM inmediato)
    Storage.addToBuffer({
      type: eventType,
      data: data,
      source: 'neurobit_tracker'
    });

    // CRÍTICO: Guardar en chrome.storage para persistencia (legacy support)
    persistLog(entry);
  }
  ...
  function saveCheckpoint(reason, content) {
    ...
    // FASE 2: Usar Storage multi-capa (CAPA 2)
    // =========================================================
    // CAPA 2: Guardar checkpoint en chrome.storage (vía Storage module)
    Storage.saveCheckpoint(`neurobit_${reason}`, {
      reason: reason,
      content: content,
      length: content.length,
    ...
    // Legacy: Mantener compatibilidad con persistencia directa
  ...
  function init() {
    ...
    // FASE 2: Inicializar Storage multi-capa (CAPA 3)
    // =========================================================
    // Configurar auto-sync a memoria_eva.jsonl
    Storage.autoSyncConfig.batchSize = 20;  // Sincronizar cada 20 eventos
    Storage.autoSyncConfig.timeInterval = 30000;  // O cada 30 segundos
    Storage.startAutoSync();
    console.log("[TRACKER] ✅ Storage multi-capa inicializado (auto-sync activo)");
