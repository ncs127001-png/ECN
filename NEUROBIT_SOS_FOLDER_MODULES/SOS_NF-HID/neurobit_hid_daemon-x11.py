...
class NeurobitHIDDaemon:
    ...
    def create_virtual_device(self):
        ...
        try:
            ...
            print(f"   Path: /dev/input/event{self.ui._device[-1]}")
            return True
    ...
    def tab_sequence(self, count=1, shift=False):
        """Secuencia de TAB (para navegación por elementos)"""
        if shift:
            self.press_key(e.KEY_LEFTSHIFT)
        
        for _ in range(count):
            self.press_key(e.KEY_TAB)
