...
class SOSDynamicTester:
    ...
    def run_dynamic_tests(self):
        ...
        # Definición de endpoints y sus payloads mínimos (Principio SINCERO: solo lo necesario)
        endpoints_to_test = [
            ("GET", "/health", None),
            ...
            ("POST", "/members/list", None) # A veces GET, a veces POST según versión, probamos GET abajo
        ]
        
        # Ajuste para members que suele ser GET con query params
        endpoints_to_test.append(("GET", "/members/list?active_only=true", None))
...
if __name__ == "__main__":
    ...
    # Si está en tools/ o SOS/, ajustar:
    # ROOT_PATH = Path(__file__).resolve().parent.parent 

    tester = SOSDynamicTester(ROOT_PATH)
    ...
    try:
        ...
        out_file = tester.generate_report(ROOT_PATH / "data" / "sos_dynamic_report.json")
        print(f"\n📊 Reporte dinámico generado en: {out_file}")
    finally:
        # Si la API no estaba corriendo antes, la detenemos ahora para no dejar basura
        if not api_was_running:
            tester.stop_api()
